...
class TCP(nn.Module):
	...
	def __init__(self, config):
		...
		# self.fuse_up_ctrl = nn.Linear(232, 256)

		self.fuse_down = nn.Linear(256, 232)
		# self.fuse_down_ctrl = nn.Linear(256, 232)

		self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
		self.fc = nn.Linear(512 , 1000)

		# self.avgpool_ctrl = nn.AdaptiveAvgPool2d((1, 1))
		# self.fc_ctrl = nn.Linear(512 , 1000)
	...
	def forward(self, img, state, target_point):
		...
		cnn_feature = self.fuse_up(cnn_feature)

		# 假设 self.transformer 是你的 Transformer 模型
		encoder_output = self.trj_encoder(cnn_feature)   #(512, 8, 256)
		...
		trj_output = self.avgpool(cnn_feature)
		trj_output = torch.flatten(trj_output, 1)
		feature_emb = self.fc(trj_output)
		...
		# # action decoder
		# fused_features_ctrl = self.fuse_up_ctrl(fused_features)
		# encoder_output_ctrl = self.ctr_encoder(fused_features_ctrl)
		# decoder_output = self.ctr_decoder(encoder_output, encoder_output_ctrl)
		# ctrl_output = decoder_output.permute(1, 0, 2)
		# ctrl_output = self.fuse_down_ctrl(ctrl_output)
		# cnn_feature_ctrl = ctrl_output.reshape(batch_size, 512, 8, -1)
		# # ctrl_output = self.avgpool_ctrl(cnn_feature)
		# # ctrl_output = torch.flatten(ctrl_output, 1)
		# # feature_emb_ctrl = self.fc_ctrl(ctrl_output)
